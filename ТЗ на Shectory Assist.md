...
**Дата:** 17.04.2026  
**Статус:** реализация MVP (**Телеграм-бот**; см. раздел «Статус выполнения»)

---

## Статус выполнения (актуализация кода и деплоя)

| Область | Статус |
|--------|--------|
| Платформа | **Telegram Bot API** (библиотека **grammy**), long poll и webhook |
| ASR / NLU / TTS (Gemini) | Реализовано (`packages/adapters/gemini`), прокси как в Ourdiary |
| Навык `gazeta_picture_of_day` | Реализовано, парсинг главной |
| Оркестратор, идемпотентность, rate limit | Реализовано |
| Профиль пользователя (голос и т.д.) | **In-memory** на инстанс; в ТЗ — SQLite/PostgreSQL — **не подключено** |
| Кэш HTML Gazeta, Redis | **Не** внедрено (опционально в ТЗ) |
| `/health` | Есть при `BOT_MODE=webhook` |
| Деплой | **hoster**, PM2 `shectory-assist-bot`, `~/shectory-assist` |
| Этапы 0–3 ТЗ | По сути закрыты на Telegram; **этап 4** — бэклог |

---

## 1. Резюме

**Shectory Assist** — голосовой ассистент в мессенджере **Telegram**, который принимает голосовые сообщения пользователя, понимает намерение, выполняет действия через подключаемые **навыки (skills)** и отвечает **голосом** (синтез речи с выбором голоса). Для распознавания и генерации речи в MVP используется **коммерческий стек Google Gemini** (подписка заказчика).
...
4. Синтезирует **голосовой ответ** выбранным голосом и отправляет **голосовое** (или допустимый в Telegram формат медиа-ответа — см. Bot API и ограничения клиентов).
...
### 4.1. Интеграция с Telegram Bot API

- Регистрация и работа **телеграм-бота** через **официальный Telegram Bot API** ([документация](https://core.telegram.org/bots/api)).
- Приём обновлений: **long polling** для локальной разработки и/или **webhook** для продакшена (HTTPS, при необходимости проверка заголовка **`X-Telegram-Bot-Api-Secret-Token`**).
- Обработка входящих **голосовых вложений**: получение `file_id` → `getFile` → загрузка по URL из ответа API, нормализация формата для пайплайна Gemini.

**Заметка по риску:** ограничения клиентов Telegram на воспроизведение форматов; при необходимости MVP допускает **голос + текст** или **аудиофайл как документ** как запасной контракт ответа.
...
1. Получить апдейт Telegram → извлечь `userId`, медиа.
2. ASR → текст.
...
| Безопасность | TLS наружу, ротация токенов Telegram/Gemini, rate limit на пользователя |
...
shectory-assist/
├── apps/
│   └── bot/                 # grammy: long poll / webhook + Fastify
├── packages/
...
**Язык реализации (рекомендация):** TypeScript (Node.js) — в проекте используется **grammy** и удобно для типизированных контрактов навыков. Допустим Python, если команда сильнее в нём: тогда отдельный HTTP-сервис и те же границы пакетов.
...
| Сервис | Назначение |
|--------|------------|
| Telegram Bot API | Вход/выход сообщений |
...
| Риск | Мера |
|------|------|
| Telegram / клиенты не воспроизводят голос так, как задумано | Проверка форматов; запасной канал ответа (текст, документ) |
...
- Регистрация бота у [@BotFather](https://t.me/BotFather), фиксация возможностей **входящего/исходящего аудио** в Telegram Bot API.
- Создание репозитория, CI (lint/test), шаблон `.env.example` без секретов.
- Минимальный echo-бот в Telegram (текст).
...
### Этап 3 — Склейка с Telegram и MVP-полировка (4–6 дней)
...
- **Телеграм-бот** — бот в мессенджере **Telegram**, доступ через официальный **Telegram Bot API**.
- **Skill** — подключаемый модуль с контрактом входа/выхода.
- **Orchestrator** — компонент, связывающий Telegram Bot API, Gemini и навыки.

---

*Документ является основой для разработки; при изменении API Telegram или политик Google разделы 4 и 6 уточняются без изменения общей идеи продукта.*
